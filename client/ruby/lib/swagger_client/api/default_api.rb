...
module SwaggerClient
  class DefaultApi
    ...
    # @return [DeviceStatus]
    def neuron(opts = {})
      data, _status_code, _headers = neuron_with_http_info(opts)
      return data
    end
    ...
    def neuron_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: DefaultApi.neuron ..."
      end
      # resource path
      local_var_path = "/node_ip:8080/v2/status"
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: DefaultApi#neuron\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
